...
exports.main = function(options, callbacks) {
  let iService = Cc["@mozilla.org/interests;1"].
  ...
  let interestWorker = new ChromeWorker("resource://gre/modules/interests/worker/interestsWorker.js");
    ...
    onAttach: function(worker) {
      ...
      worker.port.on("message", details => {
        if (details.command == "service") {
          ...
          InterestsStorage[details.data.method].apply(InterestsStorage, details.data.args).then(result => {
            let message = {
              type: "storage",
              content: {
                topic: details.data.topic,
          ...
          details.data.args.forEach(args => {
            promises.push(InterestsStorage[details.data.method].apply(InterestsStorage, args));
          });
          gatherPromises(promises).then(results => {
            /*
            iService.getInterestsByNamespace("", {
              checkSharable: true,
              excludeMeta: true,
              interestLimit: 5,
              roundDiversity: true,
            ...
            InterestsStorage._execute("SELECT * FROM moz_interests_visits", {
              columns: ["interest_id", "host", "visits"]
              }
            ).then(results => {
              console.log(JSON.stringify(results));
            });
